...
export function createSearchTool(fullModel: string) {
  return tool({
    ...
    execute: async ({
      ...
      // Use the original query as is - any provider-specific handling will be d
one in the provider
      ...
      console.log(
        `Using search API: ${searchAPI}, Search Depth: ${effectiveSearchDepthFor
API}`
      )
      ...
      try {
        if (
          ...
          if (!response.ok) {
            throw new Error(
              `Advanced search API error: ${response.status} ${response.statusTe
xt}`
            )
          }
