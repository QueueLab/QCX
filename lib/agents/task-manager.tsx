...
export async function taskManager(messages: CoreMessage[]) {
  try {
    const result = await generateObject({
      model: getModel('reasoning') as LanguageModel,
...
When making your choice, you must also provide a brief, clear explanation of your reasoning. This reasoning will be shown to the user to help them understand your decision-making process.
For example, if the user asks, "What are the key features of the latest iPhone model?", you may choose to "proceed" as the query is clear and can be answered effectively with web research alone. Your reasoning might be: "The user's query is specific and can be answered with a direct web search."
However, if the user asks, "What's the best smartphone for my needs?", you may opt to "inquire" and present a form asking about their specific requirements, budget, and preferred features to provide a more tailored recommendation. Your reasoning could be: "The user's query is broad. To provide the best recommendation, I need more information about their specific needs and preferences."
