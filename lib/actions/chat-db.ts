  ...
  return db.transaction(async (tx) => {
    ...
    if (chatId) { // If chat ID is provided, assume update or append messages
      ...
      if (!existingChat.length) {
        ...
        const newChatResult = await tx.insert(chats).values({ ...chatData, id: chatId }).returning({ id: chats.id });
      ...
      const id = nanoid();
      const newChatResult = await tx.insert(chats).values({ ...chatData, id }).returning({ id: chats.id });
      chatId = newChatResult[0].id;
    }
...
export async function createMessage(messageData: NewMessage): Promise<Message | null> {
  ...
  try {
    const result = await db.insert(messages).values({ ...messageData, id: messageData.id || nanoid() }).returning();
    return result[0] || null;
  } catch (error) {
